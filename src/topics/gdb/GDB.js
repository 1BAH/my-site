...
const GDB = () => {
    ...
    return (
        <>
            ...
            <div className={textStyles.block}>
                Чтобы посмотреть код используется утилита <div className={textStyles.highlight}>(gdb) list</div>. <br />
                <br /> Часто код собирается из нескольких файлов, и реализации функции расположена в другом файле. Как посмотреть на ее код? Применяем
                команду&nbsp;
                <div className={textStyles.highlight}>(gdb) l [function name]</div>
                . <br /> <br /> Для отладки кода иногда помогает дизассемблирование: для этого используется утилита{' '}
                <div className={textStyles.highlight}>(gdb) disassemble</div>. По умолчанию вывод будет в AT&T формате, для вывода в intel синтаксисе
                необходимо установить тип синтаксиса командой <div className={textStyles.highlight}>set disassemly-flavor intel</div>
                обратно вернуться можно с помощью команды <div className={textStyles.highlight}>set disassemly-flavor att</div>.
            </div>
            ...
            <div className={textStyles.block}>
                ...
                <div className={textStyles.quote}>
                    Так как переменная должна существовать для ее отслеживания, то watchpoint можно создать только в запущенном приложении, при
                    условии того, что переменная существует в текущей области видимости - иначе ничего не получится.
                </div>
            </div>

            <h1 className={textStyles.header}>Просмотр различной информации</h1>
            <div className={textStyles.block}>
                Основной утилитой для просмотра каких-то сведений является <div className={textStyles.highlight}>(gdb) info</div>. Если самостоятельно
                ее вызвать без каких-либо параметров, то выведется список всех возможных параметров с описанием того, что они означают. <br /> <br />{' '}
                Для просмотра всех точек останова на текущий момент используется <div className={textStyles.highlight}>(gdb) info breakpoints</div>{' '}
                или сокращенно: <div className={textStyles.highlight}>(gdb) i b</div>. <br /> <br /> Чтобы посмотреть все переменные используйте
                команду <div className={textStyles.highlight}>(gdb) info locals</div>- сокращенно{' '}
                <div className={textStyles.highlight}>(gdb) i lo</div>.
            </div>
            ...
            <div className={textStyles.block}>
                ...
                <div className={textStyles.highlight}>(gdb) p [pointer]</div>
                выдаст информацию о типе указателя и адресе:{' '}
            ...
            <div className={textStyles.block}>
                ...
                Статические массивы с фиксированным размером при попытке вызова функции <div className={textStyles.highlight}>print</div> просто
                выдадут все свои элементы:
                <div className={textStyles.code}>
                ...
                <div className={textStyles.highlight}>print</div>
